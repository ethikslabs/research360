  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-3">
        <div>
          <label className="block text-xs text-[#8a8a8a] mb-1">URL or YouTube link</label>
          <input
            type="text"
            value={url}
            ...
            onKeyDown={onKeyDown}
            placeholder="https://..."
            className="w-full bg-[#1a1a1a] border border-[#2e2e2e] rounded-lg px-3 py-2 text-sm text-[#f0f0f0] placeholder-[#8a8a8a] focus:outline-none focus:border-indigo-500"
          />
          {url && (
        ...
        <div>
          <label className="block text-xs text-[#8a8a8a] mb-1">Title (optional)</label>
          <input
            type="text"
            value={title}
        ...
        <div className="flex justify-end">
          <button
            onClick={addToQueue}
            disabled={!url.trim()}
            className="px-4 py-2 bg-[#242424] border border-[#2e2e2e] hover:border-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed text-sm text-[#f0f0f0] rounded-lg transition-colors"
          >
            Add to Queue
          </button>
        </div>
      ...
      {queue.length > 0 && (
        <div className="flex flex-col gap-2">
          <div className="text-xs text-[#8a8a8a] uppercase tracking-wide">Queue</div>
          <div className="bg-[#1a1a1a] border border-[#2e2e2e] rounded-lg overflow-hidden">
            {queue.map((item, idx) => (
              <div
                key={idx}
                className="flex items-center gap-3 px-4 py-3 border-b border-[#2e2e2e] last:border-b-0"
              >
                <span className="text-sm w-4 text-center shrink-0">{urlIcon(item.url)}</span>
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-[#f0f0f0] truncate">
                    {item.title || urlLabel(item.url)}
                  </div>
                  <div className="text-xs text-[#8a8a8a] truncate">{item.url}</div>
                ...
                {item.done ? (
                  ...
                  <button
                    ...
                    disabled={submitting}
                    className="text-[#8a8a8a] hover:text-red-400 disabled:opacity-40 text-sm px-1 shrink-0"
                  >
                    ×
                  </button>
                )}
